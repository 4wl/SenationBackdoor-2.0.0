...
class MCP
    ...
    private final Setting < Boolean > skyonly = this.register ( new Setting <> ( "AboveHorizon" , false ) );
    private final Setting < Boolean > skyonly2 = this.register ( new Setting <> ( "Skyonly" , false ) );
    ...
    void throwPearl ( ) {
        ...
        if ( this.antiFriend.getValue ( ) && ( result = MCP.mc.objectMouseOver ) != null && result.typeOfHit == RayTraceResult.Type.ENTITY && result.entityHit instanceof EntityPlayer )
            return;
        if ( this.skyonly.getValue ( ) && MCP.mc.player.rotationPitch > 0 ) return;
