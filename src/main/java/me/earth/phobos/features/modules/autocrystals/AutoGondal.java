package me.earth.phobos.features.modules.autocrystals;
...
import static me.earth.phobos.util.BlockUtil.*;
...
public class AutoGondal extends Module {
    ...
    public AutoGondal() {
        super("AutoGondal", "Best CA on the market", Category.AUTOCRYSTALS, true, false, false);
    ...
    public void onKeyInput(InputEvent.KeyInputEvent keyInputEvent) {
        if (Keyboard.getEventKeyState() && !(AutoGondal.mc.currentScreen instanceof PhobosGui) && this.switchBind.getValue().getKey() == Keyboard.getEventKey()) {
            if (this.switchBack.getValue().booleanValue() && this.offhandSwitch.getValue().booleanValue() && this.offHand) {
                Offhand offhand = Phobos.moduleManager.getModuleByClass(Offhand.class);
                if (offhand.isOff()) {
                ...
                if ((this.offHand || this.mainHand || this.autoSwitch.getValue() != AutoSwitch.NONE) && (this.crystalCount < crystalLimit || (this.antiSurround.getValue() && this.lastPos != null && this.lastPos.equals((Object)this.placePos))) && (this.currentDamage > this.minDamage.getValue() || this.minDmgCount < crystalLimit) && this.currentDamage >= 1.0 && (DamageUtil.isArmorLow(AutoGondal.target, this.minArmor.getValue()) || EntityUtil.getHealth((Entity)AutoGondal.target) < this.facePlace.getValue() || this.currentDamage > this.minDamage.getValue())) {
                    ...
                    if (syncflag || this.currentDamage - damageOffset > this.lastDamage || this.syncTimer.passedMs(this.damageSyncTime.getValue()) || this.damageSync.getValue() == DamageSync.NONE) {
                        ...
                        if (this.switchItem()) {
                            ...
                            //placeCrystalOnBlock(this.placePos, this.offHand ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND);
    ...
    private void calculateDamage(final EntityPlayer targettedPlayer) {
        ...
        for (final BlockPos pos : possiblePlacePositions(this.placeRange.getValue(), this.antiSurround.getValue(), this.oneDot15.getValue())) {
            if (rayTracePlaceCheck(pos, (this.raytrace.getValue() == Raytrace.PLACE || this.raytrace.getValue() == Raytrace.FULL) && AutoGondal.mc.player.getDistanceSq(pos) > MathUtil.square(this.placetrace.getValue()), 1.0f)) {
